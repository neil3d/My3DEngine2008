...
#pragma warning( disable : 4018 )  //signed / unsigned mismatch with vc7.1
							...
							NvStripInfoVec &outStrips, NvFaceInfoVec& outFaceList)
{
	...
	cacheSize = (in_cacheSize - CACHE_INEFFICIENCY) > 1 ? in_cacheSize - CACHE_INEFFICIENCY : 1;
...
//    outStrips.push_back(tempStrips[i]);
	...
	if(tempStrips2.size() != 0)
	{
		...
		//special optimization data structure
		std::map<int, std::vector<int> > optMap;  //mapping of <vertexIndex -> stripIndex>
		...
		for(i = 0; i < tempStrips2.size(); i++)
		{
			...
			{
				NvFaceInfo* currentFace = tempStrips2[i]->m_faces[j];
				numNeighbors += NumNeighbors(currentFace, edgeInfos);

				//while we're looping here, let's add this strip (index) to the optimization map
				std::vector<int>::iterator it = std::find(optMap[currentFace->m_v0].begin(), optMap[currentFace->m_v0].end(), i);
				if (it == optMap[currentFace->m_v0].end())
					optMap[currentFace->m_v0].push_back(i);

				it = std::find(optMap[currentFace->m_v1].begin(), optMap[currentFace->m_v1].end(), i);
				if (it == optMap[currentFace->m_v1].end())
					optMap[currentFace->m_v1].push_back(i);

				it = std::find(optMap[currentFace->m_v2].begin(), optMap[currentFace->m_v2].end(), i);
				if (it == optMap[currentFace->m_v2].end())
					optMap[currentFace->m_v2].push_back(i);
			}
		...
		bool bWantsCW = (tempStrips2[bestIndex]->m_faces.size() % 2) == 0;
		...
		while(1)
		{
			...
			NvStripInfo* lastStrip = tempStrips2[bestIndex];

			//build list of strips that could possibly give the last one good caching
			std::vector<int> potentialStripIndices;
			for (int i = 0; i < lastStrip->m_faces.size(); ++i)
			{
				NvFaceInfo* currFace = lastStrip->m_faces[i];
				for (int j = 0; j < optMap[currFace->m_v0].size(); ++j)
				{
					int currStripIndex = optMap[currFace->m_v0][j];
					std::vector<int>::iterator it = std::find(potentialStripIndices.begin(), potentialStripIndices.end(), currStripIndex);
					if (it == potentialStripIndices.end())
						potentialStripIndices.push_back(currStripIndex);
				}
				
				for (int j = 0; j < optMap[currFace->m_v1].size(); ++j)
				{
					int currStripIndex = optMap[currFace->m_v1][j];
					std::vector<int>::iterator it = std::find(potentialStripIndices.begin(), potentialStripIndices.end(), currStripIndex);
					if (it == potentialStripIndices.end())
						potentialStripIndices.push_back(currStripIndex);
				}
				
				for (int j = 0; j < optMap[currFace->m_v2].size(); ++j)
				{
					int currStripIndex = optMap[currFace->m_v2][j];
					std::vector<int>::iterator it = std::find(potentialStripIndices.begin(), potentialStripIndices.end(), currStripIndex);
					if (it == potentialStripIndices.end())
						potentialStripIndices.push_back(currStripIndex);
				}
			...
			//now look through potential strips for optimal one
			for (int i = 0; i < potentialStripIndices.size(); ++i)
			{
				int currIndex = potentialStripIndices[i];
				if (tempStrips2[currIndex]->visited)
					continue;
				
				numHits = CalcNumHitsStrip(vcache, tempStrips2[currIndex]);
				if(numHits > bestNumHits)
				{
					bestNumHits = numHits;
					bestIndex = currIndex;
				...
				else if(numHits >= bestNumHits)
				{
					...
					NvStripInfo *strip = tempStrips2[currIndex];
					int nStripFaceCount = strip->m_faces.size();
			...
			if(bestNumHits == -1.0f)
			{
				//find another unvisited one
				for (int k = 0; k < tempStrips2.size(); ++k)
				{
					if (!tempStrips2[k]->visited)
						break;
				}
				if (k == tempStrips2.size())
					break;
				else
					bestIndex = k;
			}
