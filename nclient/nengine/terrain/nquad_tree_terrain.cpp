...
namespace nexus
{
	...
	void nquad_tree_terrain::create(nheight_map16* hmap, size_t chunk_size)
	{
		m_heightmap = hmap;
		...
		m_heightmap_tex->create(hmap->get_width(), hmap->get_height());
	...
	void nquad_tree_terrain::create_shared_mesh()
	{
		//-- create mesh with skirt
		{
			...
			vert_data.init(m_chunk_size, m_chunk_size, true);
			fill_indices(index_data);
	...
	void nquad_tree_terrain::render(const view_info& view)
	{
	...
	void nquad_tree_terrain::render_quad(float minU, float minV, float maxU, float maxV, int level, float scale, const view_info& view)
	{
		...
		vector3 map_scale = m_scale;
		map_scale .x *= m_heightmap->get_width();
		map_scale .z *= m_heightmap->get_height();
	...
	void nquad_tree_terrain::draw_mesh(float uBias, float vBias, float scale, int level)
	{
		...
		{
			proxy->m_mesh.init(m_wireframe_mesh.get(), m_heightmap_tex.get());
		}
		else
		{
			proxy->m_mesh.init(m_chunk_mesh.get(), m_heightmap_tex.get());
		}
	...
	bool nquad_tree_terrain::lod_split(float minU, float minV, float maxU, float maxV, const view_info& view)
	{
		...
		float edge_len = std::max(rc.get_width()*m_scale.x, rc.get_height()*m_scale.z);
	...
	void nquad_tree_terrain::set_scale(const vector3& scale)
	{
		m_scale = scale;
	...
	void nquad_tree_terrain::set_position(const vector3& pos)
	{
		m_pos = pos;
	...
	{
		// todo : 计算高度
		float big_num = 512*1024;

		float w = m_heightmap->get_width()*m_scale.x;
		float h = m_heightmap->get_height()*m_scale.z;

		m_bounds.box_extent = vector3(w, big_num, h);
		m_bounds.origin = m_pos;
		m_bounds.sphere_radius = big_num;
	}

	void nquad_tree_terrain::update_world_matrix()
	{
		mat_set_translation(m_mat_world, m_pos.x, m_pos.y, m_pos.z);
		mat_scale(m_mat_world, m_scale.x*m_heightmap->get_width(), m_scale.y, m_scale.z*m_heightmap->get_height());
	}
