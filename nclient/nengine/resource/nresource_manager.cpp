...
namespace nexus
{
	...
	void nresource_manager::init()
	{
		...
		nmaterial_template::ptr mtl_tmp = load_material_template_script(
			resource_location(nengine::instance()->get_engine_package_name(),_T("material/default.hlsl"))
			);

		m_default_mtl.create(mtl_tmp);

		nengine::instance()->get_render_res_mgr()->set_default_material(&m_default_mtl);
	}
