...
namespace nexus
{
	const float TRN_BRUSH_CURSOR_ZBIAS = 0.04;
	//-----------------------------------------------------------------------------------------------
	...
	void nterrain_editor::draw_widgets(nrender_primitive_draw_interface* PDI)
	{
		...
		if( trn )
		{
			...
			PDI->draw_debug_string(2,2, oss.str(), color4ub(255,255,250,255));
		}
	}
	...
	void nterrain_editor::splat_set_texture(size_t layer_index, const resource_location& texture_loc)
	{
		nterrain_actor::ptr trn(m_trn_actor);
		if( !trn )
			return;
