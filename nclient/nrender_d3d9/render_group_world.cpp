...
namespace nexus
{
	...
	void render_group_world::draw_proxy_vector(proxy_vector& pv, const d3d_view_info* view, drawing_policy* dp)
	{
	...
	void render_group_world::draw_all(const d3d_view_info* view, drawing_policy* dp)
	{
		for(size_t i=0; i<ETrans_Max; i++)
		{
			draw_proxy_vector(m_proxy_vector[i], view, dp);
		}
	}
	...
	void render_group_world::draw_opaque(const d3d_view_info* view, drawing_policy* dp)
	{
		draw_proxy_vector(m_proxy_vector[ETrans_Opaque], view, dp);
		draw_proxy_vector(m_proxy_vector[ETrans_AlphaMasked], view, dp);
	}

	void render_group_world::draw_unlit_trans(const d3d_view_info* view, drawing_policy* dp)
	{
		draw_proxy_vector(m_proxy_vector[ETrans_UnlitTrans], view, dp);
	}
	
	void render_group_world::draw_mixed_trans(const d3d_view_info* view, drawing_policy* dp)
	{
		draw_proxy_vector(m_proxy_vector[ETrans_Mixed], view, dp);		
	}
