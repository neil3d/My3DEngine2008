...
{
	//nexus::matrix44 Matrix::ToNative( Matrix matrix )
	//{
	//	nexus::matrix44 result;
	...
	//Matrix Matrix::FromNative( const nexus::matrix44& matrix )
	//{
	//	Matrix result;
	...
	void Matrix::Invert()
	{
		...
		nexus::matrix44* nthis = (nexus::matrix44*)pinnedThis;

		(*nthis) = nexus::mat_inverse( *nthis );		
	}
	...
	Matrix Matrix::LookAtLH(Vector3 eye, Vector3 target, Vector3 up )
	{
		...
		nexus::mat_set_lookat_LH( *((nexus::matrix44*)pinResult),
			(*(nexus::vector3*)pinEye),
			(*(nexus::vector3*)pinTarget),
			(*(nexus::vector3*)pinUp)
			);
	...
	{
		Matrix result;
		nexus::mat_set_ortho_LH( *((nexus::matrix44*)&result),
	...
	{
		pin_ptr<Matrix> pinResult = &result;
		nexus::mat_set_ortho_LH( *((nexus::matrix44*)pinResult),
			width, height, znear, zfar);
	}
	...
	{
		Matrix result;
		nexus::mat_set_perspective_LH( *((nexus::matrix44*)&result),
	...
	{
		pin_ptr<Matrix> pinResult = &result;
		nexus::mat_set_perspective_LH( *((nexus::matrix44*)pinResult),
			fov, aspect, znear, zfar);
	}
	...
	Matrix Matrix::Invert( Matrix mat )
	{
		...
		matrix44* nresult = (matrix44*)pinResult;
		matrix44* nmat = (matrix44*)pinMatrix;

		(*nresult) = nexus::mat_inverse(*nmat);
		return result;
	}
