...
namespace nexus
{
	...
	*/
	...
	public:
		...
		void create(nheight_map16* hmap, nrender_heightmap*	heightmap_tex, size_t chunk_size);
	...
	protected:
		void render_quad(float minU, float minV, float maxU, float maxV, int level, float scale, const nviewport& view);
		bool lod_split(float minU, float minV, float maxU, float maxV, const nviewport& view);
	...
	private:
		struct trn_render_proxy
			: public primitive_proxy
		{			
			typedef shared_ptr<trn_render_proxy> ptr;

			nquad_tree_terrain* get_trn() const
			{
				return (nquad_tree_terrain*)m_comp;;
			}
			...
			virtual nmaterial_base* get_material(int lod, int mtl_id) const
			{
				(void)lod;
				(void)mtl_id;
				return get_trn()->get_material();
			}
			virtual transparent_flag get_transparent_flag() const	{	return get_trn()->get_transparent_flag(); }		
