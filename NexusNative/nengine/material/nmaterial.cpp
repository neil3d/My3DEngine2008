...
namespace nexus
{
	...
	void nmaterial::create(const resource_location& tmp_loc)
	{
		create(
			nresource_manager::instance()->load_material_template_script(tmp_loc, EIO_Block)
			);
	}
	...
	void nmaterial::set_int_param(const nstring& param_name, int val)
	{
	...
	void nmaterial::draw_effect_param(nshading_effect* effect_ptr) const
	{
		for(size_t i=0; i<m_parameter_array.size(); i++)
		{
			...
			{
			case EMPT_Int:
				effect_ptr->set_int(param_val.name, param_val.int_value);
				break;
			case EMPT_Float:
