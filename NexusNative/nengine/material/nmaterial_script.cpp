...
namespace nexus
{
	...
	{		
		...
		{
			nthrow2(_T("load material template script failed"), loc.to_string().c_str());
		}

		//-- 分析shader，取得所有参数的类型、名称
		parse_material_parameter_from_HLSL();
		prase_config_from_HLSL();
		prase_define_from_HLSL();
	...
	void nmaterial_script::prase_define_from_HLSL()
	{
		//-- todo ： 如果是在注释中呢？
		std::string::size_type f = m_shader_code.find("MTL_TWO_SIDED");
