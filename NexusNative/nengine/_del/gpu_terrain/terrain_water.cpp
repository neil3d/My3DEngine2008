...
namespace nexus
{
	...
	void nterrain_water::_on_device_lost(int param)
	{
		m_share_mesh.reset();
	}

	bool nterrain_water::_on_device_reset(int param)
	{
		create_renderer_data();
		return true;
	}

	void nterrain_water::create_renderer_data()
	{
		//-- create share mesh
		...
		vert_data.init(m_chunk_size, m_chunk_size);
		...
		m_root_node->create(root_rc, this, m_chunk_size);
	}
}//namespace nexus