...
#ifndef _NEXUS_CORE_GRINNINGLIZARD_MICROPATHER_H_
#define _NEXUS_CORE_GRINNINGLIZARD_MICROPATHER_H_
	...
	*/
	class PathNode
	{
	  public:
		...
		void CheckList()
		{
			nASSERT( totalCost == FLT_MAX );
			for( PathNode* it = next; it != this; it=it->next ) {
				nASSERT( it->prev == this || it->totalCost >= it->prev->totalCost );
				nASSERT( it->totalCost <= it->next->totalCost );
			}
		}
		#endif
	...
	public:
		...
		void GetCache( int start, int nNodes, NodeCost* nodes ) {
			nASSERT( start >= 0 && start < cacheCap );
			nASSERT( nNodes > 0 );
			nASSERT( start + nNodes <= cacheCap );
