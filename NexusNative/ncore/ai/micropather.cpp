...
void OpenQueue::Push( PathNode* pNode )
{
	...
	nASSERT( pNode->inOpen == 0 );
	nASSERT( pNode->inClosed == 0 );
	...
	nASSERT( pNode->totalCost < FLT_MAX );
	...
	nASSERT( pNode->inOpen );	// make sure this was actually added.
#ifdef _DEBUG
	sentinel->CheckList();
#endif
}
...
PathNode* PathNodePool::Alloc()
{
	if ( freeMemSentinel.next == &freeMemSentinel ) {
		nASSERT( nAvailable == 0 );

		Block* b = NewBlock();
		b->nextBlock = blocks;
		blocks = b;
		nASSERT( freeMemSentinel.next != &freeMemSentinel );
	}
	...
	else if ( node->cacheIndex < 0 )
	{
		...
		#ifdef _DEBUG
		{
			...
			for ( unsigned i=0; i<stateCostVec.size(); ++i )
				nASSERT( stateCostVec[i].state != node->state );
		}
		#endif
	...
	while ( !open.Empty() )
	{
		...
		else
		{
			...
			for( int i=0; i<node->numAdjacent; ++i )
			{
				...
				nASSERT( inEither != node );
				nASSERT( !( inOpen && inClosed ) );
				...
				if ( inEither ) {
					...
					nASSERT( !child->inOpen && !child->inClosed );
					open.Push( child );
				}
			}
		}					
...
int MicroPather::SolveForNearStates( void* startState, std::vector< StateCost >* near, float maxCost )
{
	...
	*/
	...
	while ( !open.Empty() )
	{
		...
		for( int i=0; i<node->numAdjacent; ++i )
		{
			nASSERT( node->costFromStart < FLT_MAX );
			float newCost = node->costFromStart + nodeCostVec[i].cost;
			...
			PathNode* child = nodeCostVec[i].node;
			nASSERT( child->state != newPathNode->state );	// should never re-process the parent.
	...
	for( unsigned i=0; i<near->size(); ++i ) {
		for( unsigned k=i+1; k<near->size(); ++k ) {
			nASSERT( near->at(i).state != near->at(k).state );
		}
	}
#endif
